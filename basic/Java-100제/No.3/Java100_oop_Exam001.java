...
// 시골에 농기계를 만들어내는 공장(Class)이 들어섰고, 농부들이 주문을 넣으면 농기계가 만들어져 나온다. --> 그 농기계가 객체
...
// 예를 들어, 어떤 농부가 경작용 트랙터를 주문한다고 하면 가격을 고민하고, 색상도 고민하고, 연식도 고민할 것이다.
// 뿐만 아니라, 트랙터의 기능이나 성능 등 동작들에 대해서도 많은 고민을 할 것이다.
// 고민하는 걸 두 개 정도로 압축하면 크게 봤을 때 "특징"과 "동작"이 된다.
// 클래스는 이러한 만들고자 하는 객체의 "특징"과 "동작"에 많은 시간을 들이고 집중하면서 설계를 하게 된다.

// [3] : 위 사항들을 조금은 어려운 말로 정의해본다면?
// 객체의 특징 --> 속성(attribute)
// 객체의 동작 --> 메서드(method) --> 함수와 거의 동일
// 참고로, 파이썬에는 일반적으로 함수라는 표현을 쓰지만, 클래스 내에서의 함수는 메서드라고 한다. 거의 함수와 동일하다.
