...
// 이 문제는 자바 문법 중 인터페이스에 대한 개념 및 어떻게 사용하는지에 대해서 아는지 묻는 문제이다.
...
// 일본의 가전기업들이 한국에서 전자제품을 팔고 싶다면, 한국 내 220v "규격"을 지켜서 만들어야만 팔 수 있다.
// 이러한 "규격"을 인터페이스라 할 수 있고, 인터페이스는 하나의 "표준화"를 제공하는 것이라 할 수 있다.

// 추상 클래스 vs 인터페이스
// (인터페이스가) 추상 클래스와 거의 비슷하다 --> 그러나, 그 추상화 정도가 더 높다(엄격하다) --> 따라서, 일반 메서드 멤버 필드(변수)를 가질 수 없다.
...
// class --> extends	interface --> implements (o), extends(x)
...
// 그러나, 인터페이스에서는 상속(extends)이라는 표현을 쓰지 않고 "구현"의 의미를 강조하는 implements 키워드를 사용하여 다중 상속을 구현한다.