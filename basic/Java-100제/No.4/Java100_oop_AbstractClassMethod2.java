...
abstract class Animal {
	...
	abstract void cry();
	void eat() {System.out.println("쳐먹다");}
}
...
class Cow extends Animal {
	void cry() {System.out.println("음무~");}
}
...
public class Java100_oop_AbstractClassMethod2 {
	public static void main (String[] args) {
		...
		// [3] : Summary
		// 추상(부모) 클래스는 다른(자식) 클래스들의 공통적인 특징을 변수나 메서드로 정의만 해놓는 것 --> 추상 메서드
		...
		// 추상 클래스에서 선언만 해놓음으로써, 이후 새로운(자식) 클래스들이 상속 받아 구현하므로 새로운 클래스를 작성할 때 하나의 틀이 된다.
		...
		// 결론?
		// 부모(추상) 클래스에서 구현하지 않는 이유는 해당 메서드의 구현이 상속받는 클래스에 따라서 달라질 수 있기 때문에 선언만 해둔 것이다.
		// 마치 돈 많은 부모가 엄청난 대지만 상속해주고 용도는 자식들이 알아서 사용해라~ 와 같다.
