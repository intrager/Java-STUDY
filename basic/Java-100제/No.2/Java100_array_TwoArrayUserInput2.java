...
public class Java100_array_TwoArrayUserInput2 {
	public static void main(String[] args) {
		...
		for(int i = 0; i < readRow; i++) {
			System.out.print( (i+1) + "행에 입력할 문자 "+ readColumn+"개를 차례대로 입력하고 [Enter] 치세요 = ");
			strAr[i] = sc.next();
			for(int j = 0; j < readColumn; j++) {
				hangMap[i][j] = strAr[i].charAt(j);	// j 인덱스에 있는 값을 하나씩 가져와서 --> hangMap 좌표에 세팅
			}
		}
		
		// [6] : 2차원 배열에 들어있는 값 출력하기
		System.out.println(strAr);	// 주소가 출력됨
		for(int k = 0; k < strAr.length ; k++)
			System.out.println(strAr[k]);
		
		System.out.println("---------------------------------");
		for(int i = 0; i < readRow; i++) {
			for(int j = 0; j < readColumn; j++)
				System.out.print(hangMap[i][j]);
			System.out.println();
		}
		System.out.println("---------------------------------");
	}
